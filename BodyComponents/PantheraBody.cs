...
namespace Panthera.BodyComponents
{
    ...
    public class PantheraBody : CharacterBody
    {
        ...
        public float energy
        {
            ...
            {
                _energy = value;
                if (ptraObj.activePreset == null) return;
                if (_energy > ptraObj.activePreset.maxEnergy)
                    _energy = ptraObj.activePreset.maxEnergy;
                if (_energy < 0)
                    _energy = 0;
        ...
        public float stamina
        {
            set
            {
                _stamina = value;
                if (ptraObj.activePreset == null) return;
                if (_stamina > ptraObj.activePreset.maxStamina)
                    _stamina = ptraObj.activePreset.maxStamina;
                if (_stamina < 0)
                    _stamina = 0;
        ...
        public float power
        {
            set
            {
                _power = value;
                if (ptraObj.activePreset == null) return;
                if (_power > ptraObj.activePreset.maxPower)
                    _power = ptraObj.activePreset.maxPower;
                if (_power < 0)
                    _power = 0;
        ...
        public float fury
        {
            set
            {
                _fury = value;
                if (ptraObj.activePreset == null) return;
                if (_fury > ptraObj.activePreset.maxFury)
                    _fury = ptraObj.activePreset.maxFury;
                if (_fury < 0)
                    _fury = 0;
        ...
        public float comboPoint
        {
            set
            {
                _comboPoint = value;
                if (ptraObj.activePreset == null) return;
                if (_comboPoint > ptraObj.activePreset.maxComboPoint)
                    _comboPoint = ptraObj.activePreset.maxComboPoint;
                if (_comboPoint < 0)
                    _comboPoint = 0;
        ...
        public float shield
        {
            set
            {
                _shield = value;
                if (ptraObj.activePreset == null) return;
                if (_shield > ptraObj.activePreset.maxShield)
                    _shield = ptraObj.activePreset.maxShield;
                if (_shield < 0)
                    _shield = 0;
                if (RoR2Application.isInMultiPlayer == true) new ServerSendFrontShield(gameObject, shield).Send(NetworkDestination.Server);
            }
            get
            {
                if (_shield > ptraObj.activePreset.maxShield)
        ...
        public void DoInit()
        {
            ...
            energy = ptraObj.activePreset.maxEnergy;
            stamina = ptraObj.activePreset.maxStamina;
            power = 0;
            fury = PantheraConfig.Fury_startingFury;
            comboPoint = 0;
            shield = ptraObj.activePreset.maxShield;
        }
        ...
        public static VisibilityLevel GetVisibilityLevelHook(Func<CharacterBody, TeamIndex, VisibilityLevel> orig, CharacterBody self, TeamIndex observerTeam)
        {
            ...
            if (self.HasBuff(Buff.StealthBuff) == true)
            {
